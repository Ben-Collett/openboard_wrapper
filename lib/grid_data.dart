...
class GridData extends Searlizable{
  ...
  Map<String,dynamic> toJson(){
   Map<String,dynamic> out = {'rows':numberOfRows,'columns':numberOfColumns};
  ...
  GridData insertRowAt(int rowIndex) {
    ...
    List<ButtonData?> newRow = List<ButtonData?>.generate(_order[0].length, (index) => null);
  ...
  GridData setButtonData({required int row, required int col, required ButtonData? data}){
    _order[row][col]= data;
    return this;
  }
     
  GridData expandToFitAndSetButton({required int row, required int col, required ButtonData? buttonData}){
