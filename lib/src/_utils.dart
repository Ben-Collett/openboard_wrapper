...
class InlineData {
  ...
  InlineData.decode(String inline) {
    ...
    dataType = inline.substring(5, semicolon); // skip 'data:'

    const String base = 'base';
    int indexOfBase = inline.indexOf(base, semicolon);
    if (indexOfBase == -1) {
      throw ('inline data doesn\'t contain $base');
    }

    int moveForward = indexOfBase + base.length;
    int comma = inline.indexOf(',', moveForward);
    encodingBase = int.parse(inline.substring(moveForward, comma));
    data = inline.substring(comma + 1);
  }
  String encode() {
